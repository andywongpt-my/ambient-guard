...
|----|------|------------|--------|------------|-------|--------|
| R1 | **Bee account/device availability for a live login.** The Fedora `meow` server has NO `bee` CLI installed and it is unknown whether it can hold a Bee login. Milestone 1 requires REAL Bee data at runtime — a README-only integration does NOT count. | High | High | RESOLVED on the dev machine (2026-09-07): `@beeai/cli` v0.7.3 installed, `bee login` verified as Andy Wong (id 50853); `bee today`/`bee locations current` return real data (evidence/M1_bee_login.md). REMAINING: server-side login on `meow` still to be spiked; if it can't hold a session, run the live backend from the dev host. | Dev | Mitigating |
